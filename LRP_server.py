...
from CreateGraphEdge_server import get_edge_list
import dgl
import matplotlib.pyplot as plt
from matplotlib.patches import Rectangle
from PIL import Image as PIL_Image
from SplitImageRegions_server import preprocess_object_names
from visual_genome import local as vg
import os
import numpy as np


def lrp(model, input, epsilon=1e-5):
...
def get_sample(image_id, label, experiment='countryVSurban'):
    features = torch.load(NODES_DIR + f'{experiment}/node-features-{image_id}-{label}.pt')
...
def random_picker(sample_id, experiment='countryVSurban'):
    ...
    if label_predicted == label:
        ...
        print("False")
    values, sorted_relevance = torch.sort(lrp(model, input))
    print(values)
    draw_sample(image_id, label, sorted_relevance, sample_id, values, experiment)
